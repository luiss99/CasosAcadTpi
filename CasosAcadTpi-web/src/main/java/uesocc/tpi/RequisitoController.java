...
public class RequisitoController implements Serializable {
    ...
    @EJB
    private uesocc.tpi.RequisitoFacade ejbFacade;
    ...
    public Requisito getSelected() {
        if (current == null) {
            ...
            current.setRequisitoPK(new uesocc.tpi.lib.RequisitoPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getRequisitoPK().setIdTipoRequisito(current.getTipoRequisito().getIdTipoRequisito());
    ...
    public Requisito getRequisito(uesocc.tpi.lib.RequisitoPK id) {
    ...
    public static class RequisitoControllerConverter implements Converter {
        ...
        uesocc.tpi.lib.RequisitoPK getKey(String value) {
            uesocc.tpi.lib.RequisitoPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new uesocc.tpi.lib.RequisitoPK();
            key.setIdRequisito(Integer.parseInt(values[0]));
            key.setIdTipoRequisito(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(uesocc.tpi.lib.RequisitoPK value) {
